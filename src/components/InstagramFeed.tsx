...
export function InstagramFeed() {
  ...
  useEffect(() => {
    const fetchInstagramFeed = async () => {
      try {
        const response = await fetch('https://feeds.behold.so/eO2DDGxcnmoR3DskK1KD');
        if (!response.ok) {
      ...
      {posts.map((post) => (
        <a key={post.id} href={post.permalink} target="_blank" rel="noopener noreferrer" className="relative group overflow-hidden rounded-2xl shadow-xl aspect-square block">
          {post.mediaType === 'VIDEO' ? (
            <>
              <video src={post.mediaUrl} autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
            ...
            <Image src={post.mediaUrl} alt="Instagram post" fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
          )}
